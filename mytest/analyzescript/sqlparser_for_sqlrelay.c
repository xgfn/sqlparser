...
static gsp_sqlparser *parser = NULL;
static gsp_sql_statement *gstmt = NULL;

static FILE *infoResult;
int sql_encrypt (gsp_sqlparser *parser, gsp_node* node, BOOL bUpdate);
			...
			if(targetValue->resultColumnList!=NULL){
				foreach(element, targetValue->resultColumnList){
                    ...
                    if (eet_simple_constant == column->expr->expressionType)
                    {
                        ...
                        {
                            printf("\033[0;31m");
                            sql_encrypt (parser, (gsp_node*)column, TRUE);
                            char sqlparsedout[2048]= {0};
                            snprintf (sqlparsedout, 2048, "%s", gsp_getNodeText((gsp_node*)gstmt->stmt));
                            printf ("sqlparserout:%s\n", sqlparsedout);
                            } 
                    }
					printf("\n\t");
					printf(gsp_node_text((gsp_node*)column));
		...
		foreach(cell, pSqlstmt->resultColumnList){
			...
			if(field->expr!=NULL && field->expr->leftOperand!=NULL && field->expr->rightOperand!=NULL ){
                ...
                if (eet_simple_constant == field->expr->rightOperand->expressionType)
                {
                    ...
                    {
                        printf("\033[0;31m");
                        sql_encrypt (parser, (gsp_node*)field->expr->rightOperand, TRUE);
                    } 

                }
				printf(gsp_node_text((gsp_node*)field->expr->rightOperand));
                
				//printf(gsp_getNodeText((gsp_node*)gstmt->stmt));
                printf("\033[0m");
...
int sqlparser(const char *sqlquery, int encrypt, char *sqlparsedout)
{
    if (NULL == sqlquery)
    {
        return 1;
...
int sql_encrypt (gsp_sqlparser *parser, gsp_node* node, BOOL bUpdate)
{
    ...
    /* input */
    char * input = gsp_getNodeText(node);
    ...
    snprintf (stAes.szAlgoName, sizeof (stAes.szAlgoName), "%s", "AES-128-CFB");
    stAes.pucKey  = szKey;
    stAes.pucIv = szIv;

    stAes.pucInput = (unsigned char*) input;
    stAes.iInputEncode = 2;
    stAes.iInputLen = inputlen;

    stAes.pucOutput = output;
    stAes.iMode = 1;
    ...
    {
        printf ("LOCKET_CIPHER_AESCrypt failed, err:%d\n", iErr);
        return iErr;
    }
    char szBuffer[2048] = {0};
    snprintf (szBuffer, sizeof (szBuffer), "%lkt%s", stAes.pucOutput);
    
    gsp_setNodeText(parser, node, szBuffer, bUpdate);
#endif
#define ERR_COMMON_FAILED 0
#define ERR_COMMON_SUCCESS 1
    int iErr = ERR_COMMON_SUCCESS;
    ...
    OpenSSL_add_all_ciphers ();
    /* Check algo */
    pstCipher = EVP_get_cipherbyname("AES-128-CFB");
    if (NULL == pstCipher)
    {
    ...
    char * input = gsp_getNodeText(node);
    int inputlen = strlen(input) - 2; //去除单引号
    ...
    do 
    {
        ...
        /* encrypt/decrypt */
        iErr = EVP_CipherUpdate(pstCipherCtx, output, &iOutLen1, input, inputlen);
        if (ERR_COMMON_SUCCESS != iErr)
        {
        ...
        int iOutLen2       = 0;
        iErr = EVP_CipherFinal_ex(pstCipherCtx, output + iOutLen1, &iOutLen2);
        if (ERR_COMMON_SUCCESS != iErr)
        {
        ...
        char output2[2048] = {0};
        int iLen = EVP_EncodeBlock(output2, output, iOutLen1 + iOutLen2);
        //printf ("base64 ilen:%d\n", iLen);


        char szBuffer[2048] = {0};
        snprintf (szBuffer, sizeof (szBuffer), "'$lkt%s'", output2);
        //printf ("base64:%s\n", szBuffer);
        gsp_setNodeText(parser, node, szBuffer, bUpdate);
    } while (0);
...
#if 1
int sql_decrypt (const char *input, char *output, int *outlen)
{
...
#define ERR_COMMON_SUCCESS 1
    ...
    char output2[2048] = {0};
    int iLen = EVP_DecodeBlock(output2, input, inputlen);
    printf ("base64 decode ilen:%d\n", iLen);
    ...
    do 
    {
        ...
        /* encrypt/decrypt */
        iErr = EVP_CipherUpdate(pstCipherCtx, output1, &iOutLen1, output2, iLen);
        if (ERR_COMMON_SUCCESS != iErr)
        {
            printf ("EVP_CipherUpdate failed\n");
            break;
        ...
        int iOutLen2       = 0;
        iErr = EVP_CipherFinal_ex(pstCipherCtx, output1 + iOutLen1, &iOutLen2);
        if (ERR_COMMON_SUCCESS != iErr)
        {
            printf ("EVP_CipherFinal_ex failed\n");
            break;
        ...
        snprintf (output, 2048, "%s", output1);
        
        *outlen = output1 + iOutLen1;
        printf ("------output:%s\n", output);
    } while (0);
